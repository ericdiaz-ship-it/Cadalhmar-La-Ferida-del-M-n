...
public class InventariManager : MonoBehaviour {
    ...
    void Awake() {
        ...
        // Assegurem que la base de dades tingui inicialitzat el lookup
        ...
        if (VariablesGlobals.documentsRecollits != null) {
            if (baseDeDades == null) {
                ...
                foreach (string id in VariablesGlobals.documentsRecollits) {
                    ...
                    // Actualitzem el text del botó directament
                    ...
                    // Configurem el script DocumentButton si existeix
                    DocumentButton docBtn = btn.GetComponent<DocumentButton>();
                    if (docBtn != null) docBtn.Setup(doc.id, doc.titol);

                    // Capturem una còpia local per evitar problemes de closure
                    var localDoc = doc;
